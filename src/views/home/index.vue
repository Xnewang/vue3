  ...
  <div class="home-container layout-pd">
    <el-row :gutter="15" class="home-card-one mb15">
      <el-col
        ...
        :key="index"
        :class="{
          'home-media home-media-lg': index > 1,
          'home-media-sm': index === 1,
        }"
      >
        <div class="home-card-item flex">
          <div
            class="flex-margin flex w100"
            :class="`home-one-animation${index}`"
          >
            <div class="flex-auto">
              <span class="font30">{{ item.num1 }}</span>
              <span class="ml5 font16" :style="{ color: item.color1 }">{{
                item.num2
              }}</span>
              <div class="mt10">{{ item.num3 }}</div>
            </div>
            <div
              class="home-card-item-icon flex"
              :style="{ background: `var(${item.color2})` }"
            >
              <i
                class="flex-margin font32"
                :class="item.num4"
                :style="{ color: `var(${item.color3})` }"
              ></i>
            </div>
          </div>
        </div>
          ...
          <div class="home-monitor">
            <div class="flex-warp">
              <div
                ...
                v-for="(item, index) in state.homeThree"
                :key="index"
              >
                <div
                  class="flex-warp-item-box"
                  :class="`home-animation${index}`"
                >
...
/**
 *  markRaw() 函数，将图表实例标记为“原始值”（raw value）。这样做的目的是告诉 Vue.js 不要对该值进行响应式处理，否则可能会导致性能问题。由于 echarts 图表本身就已经具有足够的交互性和动态性，不需要额外的响应式处理，因此使用 markRaw() 可以避免不必要的性能开销。
 */
...
// 饼图
const initPieChart = () => {
  state.global.homeChartTwo = echarts.init(homePieRef.value);
  var getname = [
    "房屋及结构物",
    "专用设备",
    "通用设备",
  ...
  const option = {
    tooltip: {
      trigger: "item",
      formatter:
        "（系列名称）：{a}  <br/> （数据项名称）：{b}<br/> （数值）：{c} <br/>（百分比）：{d}%",
    },
    /**
   * 图例组件。
...
// 柱状图
const initBarChart = () => {
  // state.global.homeChartThree = echarts.init(homeBarRef.value);
  ...
  .home-card-one {
    @for $i from 0 through 3 {
      .home-one-animation#{$i} {
        /*
					opacity: 0; 设置透明度为 0，即元素完全透明。
					animation-name: error-num; 设置动画名称为 error-num，这意味着在应用该样式的元素上将触发名为 error-num 的动画。
					animation-duration: 0.5s; 设置动画持续时间为 0.5 秒。
					animation-fill-mode: forwards; 当动画完成后，保持最后一个关键帧的样式，以确保元素停留在动画结束时的状态。
					animation-delay: calc($i/4) + s; 设置动画延迟时间，使用 calc() 函数和插值语法将 $i 的值除以 4，并添加 s 表示秒的单位。这样，循环的第一个元素的动画延迟为 0s，第二个元素为 0.25s，第三个元素为 0.5s，最后一个元素为 0.75s。
				*/
        opacity: 0;
